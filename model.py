...
#     # our input size is 28x28, the sequential MNIST dataset
...
#         self.fc1 = nn.Linear(1, 256, bias=bias_flag)  # modify the input size to match the MNIST dataset
#         self.bntt1 = nn.ModuleList([nn.BatchNorm1d(256, eps=1e-4, momentum=0.1, affine=affine_flag) for _ in range(self.batch_num)])
#         self.fc2 = nn.Linear(256, 256, bias=bias_flag)
#         self.bntt2 = nn.ModuleList([nn.BatchNorm1d(256, eps=1e-4, momentum=0.1, affine=affine_flag) for _ in range(self.batch_num)])
#         self.fc3 = nn.Linear(256, num_cls, bias=bias_flag)

#         self.fc_list = [self.fc1, self.fc2, self.fc3]
#         self.bntt_list = [self.bntt1, self.bntt2]
...
#         mem_fc2 = torch.zeros(batch_size, 256).cuda()
#         mem_fc3 = torch.zeros(batch_size, self.num_cls).cuda()
#         mem_fc_list = [mem_fc1, mem_fc2, mem_fc3]

#         for t in range(self.num_steps):
#             spike_inp = PoissonGen(inp[:, t].unsqueeze(1))
#             out_prev = spike_inp

#             for i in range(len(self.fc_list)):
#                 if t < len(self.bntt_list[i]):
#                     mem_fc_list[i] = self.leak_mem * mem_fc_list[i] + self.bntt_list[i][t](self.fc_list[i](out_prev))
...
#         out_voltage = mem_fc3 / self.num_steps
