...
class SNN_VGG9_BNTT(nn.Module):
    ...
    def forward(self, inp):
        ...
        for t in range(self.num_steps):
            ...
            for i in range(len(self.conv_list)):
                ...
                # Output Spike Rate
                if self.current_epoch == 100 and t == self.num_steps - 1:
                    spikes_num = torch.sum(out == 1)
                    conv_str = "Conv " + str(i) + " spikes num: " + str(spikes_num) + "\n"
                    with open('spikeoutput.txt', 'a') as f:
                        f.write(conv_str)

                    print("Complete Conv " + str(i) + "!")
            ...
            # Output Spike Rate
            if self.current_epoch == 100 and t == self.num_steps - 1:
                fcspikes_num = torch.sum(out == 1)
                fc_str = "FC " + "spikes num: " + str(fcspikes_num) + "\n"
                with open('spikeoutput.txt', 'a') as f:
                    f.write(fc_str)
                print("Complete FC!")
    ...
    def print_gamma_weight(self):
            with open('learnableweights.txt', 'w') as f:
                for bntt_index, bntt_layer in enumerate(self.bntt_list, start=0):
                    for bn_index, bn_layer in enumerate(bntt_layer, start=0):
                        weights = bntt_layer[bn_index].weight.data  # Convert to numpy array
                        weights_rounded = torch.round(weights * 1000) / 1000  # Round to 3 decimal places
                        weights_avg = torch.mean(weights_rounded)  # Calculate the average
                        f.write(f'Timestep: {bn_index}, BNTT Layer: {bntt_index}, BN Layer: {bn_index},Average Weights: {weights_avg} Weights: {weights}\n')
